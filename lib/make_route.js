'use strict'


module.exports = function(cuk) {
  const { _, helper } = cuk.lib

  return function(file, cuksPkg, pkg, router, appDir) {
    const disabled = _.get(cuk.pkg.route, 'cfg.common.disabled', [])
    let routePath = file.replace(appDir, '').replace('.js', ''),
      routes = require(file)(cuk)
    let _name = `${cuksPkg.id}:${routePath}`
    if (disabled.indexOf(_name) > -1) {
      helper('core:bootTrace')('%B Disabled %K %s', null, null, _name)
      return
    }
    if (routes instanceof pkg.lib.Router) {
      router.use(routePath, routes.routes(), routes.allowedMethods())
      helper('core:bootTrace')('%B Enabled %K %s %L [Router] %s%s', null, null, _name, null, cuksPkg.cfg.common.mount === '/' ? '' : cuksPkg.cfg.common.mount, routePath)
      return
    }
    if (_.isString(routes)) {
      let name = `GET:${_name}`
      const isRaw = routes.substr(0, 5) === 'view:'
      const content = isRaw ? routes.substr(5) : routes
      if (cuk.pkg.view)
        router.get(name, ctx => {
          if (isRaw)
            ctx.render(routes)
          else
            ctx.body = content
        })
      else
        router.get(name, ctx => {
          ctx.body = content
        })

        helper('core:bootTrace')('%B Enabled %K %s %L [GET] %s%s', null, null, _name, null, cuksPkg.cfg.common.mount === '/' ? '' : cuksPkg.cfg.common.mount, routePath)
      return
    }
    if (_.isPlainObject(routes))
      if (routes.route) {
        if (routes.middleware)
          router.use(helper('http:composeMiddleware')(routes.middleware, 'router', true))
        routes = routes.route
      } else {
        routes = [routes]
      }
    _.each(routes, r => {
      let method = r.method || 'get',
        rpath = r.path || routePath,
        methods = helper('core:makeChoices')(method)
      let parts = rpath.split('/')
      parts.shift()
      if (pkg.cfg.common.trimIndex && _.last(parts) === 'index') {
        if (pkg.cfg.common.ext === '') {
          rpath = '/' + parts.join('/')
        } else {
          parts.pop()
          rpath = '/' + parts.join('/') + (parts.length === 0 ? '':'/')
        }
      } else {
        rpath = '/' + parts.join('/') + pkg.cfg.common.ext
      }
      let mws = helper('http:composeMiddleware')(r.middleware, `${pkg.id}:${cuksPkg.id}:${routePath}:${_.map(methods, m => m.toUpperCase()).join(',')}`, true)
      _.each(methods, m => {
        let name = `${m}:${cuksPkg.id}:${routePath}`
        if (r.redirect) {
          let dest = _.isString(r.redirect) ? r.redirect : (r.destination || name)
          router.all(rpath, mws, ctx =>{
            ctx.redirect(dest)
            ctx.status = 301
          })
          helper('core:bootTrace')('%B Enabled %K %s %L [%s:Redirect] %s%s → %s', null, null, _name, null, m.toUpperCase(), cuksPkg.cfg.common.mount === '/' ? '' : cuksPkg.cfg.common.mount, rpath, dest)
        } else {
          if (_.isPlainObject(r.param)) {
            _.forOwn(r.param, (v, k) => {
              router.param(k, v)
            })
          }
          router[m](name, rpath, mws, r.handler)
          helper('core:bootTrace')('%B Enabled %K %s %L [%s] %s%s', null, null, _name, null, m.toUpperCase(), cuksPkg.cfg.common.mount === '/' ? '' : cuksPkg.cfg.common.mount, rpath)
        }
      })
    })
  }

}