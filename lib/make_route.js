...
module.exports = function(cuk) {
  ...
  return function(file, cuksPkg, pkg, router, appDir) {
    ...
    if (disabled.indexOf(_name) > -1) {
      helper('core:bootTrace')('|  |  |- Disabled => %s', _name)
      return
    }
    if (routes instanceof pkg.lib.Router) {
      router.use(routePath, routes.routes(), routes.allowedMethods())
      helper('core:bootTrace')('|  |  |- Enabled => %s -> [Router] %s%s', _name, cuksPkg.cfg.common.mount === '/' ? '' : cuksPkg.cfg.common.mount, routePath)
      return
    }
    if (_.isString(routes)) {
      ...
      helper('core:bootTrace')('|  |  |- Enabled => %s -> [GET] %s%s', _name, cuksPkg.cfg.common.mount === '/' ? '' : cuksPkg.cfg.common.mount, routePath)
      return
    }
    ...
    _.each(routes, r => {
      ...
      _.each(methods, m => {
        ...
        if (r.redirect) {
          ...
          helper('core:bootTrace')('|  |  |- Enabled => %s -> [%s:Redirect] %s%s -> %s', _name, m.toUpperCase(), cuksPkg.cfg.common.mount === '/' ? '' : cuksPkg.cfg.common.mount, rpath, dest)
        } else {
          ...
          helper('core:bootTrace')('|  |  |- Enabled => %s -> [%s] %s%s', _name, m.toUpperCase(), cuksPkg.cfg.common.mount === '/' ? '' : cuksPkg.cfg.common.mount, rpath)
        }
      })
    })
  }
