...
module.exports = function(cuk) {
  ...
  return function(file, cuksPkg, pkg, router, rootDir) {
    ...
    if (_.isString(routes)) {
      const name = `${m}:${cuksPkg.id}:${routePath}`
      ...
      pkg.trace('Serve » GET -> %s:%s%s', cuksPkg.id, cuksPkg.cfg.common.mount === '/' ? '' : cuksPkg.cfg.common.mount, routePath)
      return
    }
    if (_.isPlainObject(routes))
      if (routes.route) {
        if (routes.middleware)
          router.use(helper('http:composeMiddleware')(routes.middleware))
        routes = routes.route
      } else {
        routes = [routes]
      }
