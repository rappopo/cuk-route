...
module.exports = function(cuk) {
  ...
  return function(file, cuksPkg, pkg, router, rootDir) {
    ...
    if (routes instanceof pkg.lib.Router) {
      ...
      pkg.trace('Serve » Router -> %s:%s%s', cuksPkg.id, cuksPkg.cfg.mount === '/' ? '' : cuksPkg.cfg.mount, routePath)
      return
    }
    ...
    _.each(routes, r => {
      ...
      _.each(methods, m => {
        ...
        if (r.redirect) {
          ...
          pkg.trace('Redirect » %s -> %s:%s%s -> %s', m.toUpperCase(), cuksPkg.id, cuksPkg.cfg.mount === '/' ? '' : cuksPkg.cfg.mount, rpath, dest)
        } else {
          ...
          pkg.trace('Serve » %s -> %s:%s%s', m.toUpperCase(), cuksPkg.id, cuksPkg.cfg.mount === '/' ? '' : cuksPkg.cfg.mount, rpath)
        }
      })
    })
  }
