  ...
  return function(file, cuksPkg, pkg, router, rootDir) {
    ...
    _.each(routes, r => {
      let method = r.method || 'get',
        ...
        methods = helper.makeChoices(method)
      ...
      if (r.redirect) {
        let dest = _.isString(r.redirect) ? r.redirect : (r.destination || r.name)
        if (mws.length === 0)
        ...
        pkg.trace('Redirect » %s%s -> %s', cuksPkg.cfg.mount === '/' ? '' : cuksPkg.cfg.mount, rpath, dest)
      } else {
