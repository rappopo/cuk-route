...
module.exports = function (cuk) {
  ...
  return function (file, cuksPkg, pkg, router, appDir) {
    const disabled = _.get(cuk.pkg.route, 'cfg.disabled', [])
    ...
    if (routes instanceof pkg.lib.Router) {
      ...
      helper('core:trace')('|  |  |- Enabled => %s -> %s%s', name, cuksPkg.cfg.mount === '/' ? '' : cuksPkg.cfg.mount, routePath)
      return
    }
    if (_.isString(routes)) {
      ...
      helper('core:trace')('|  |  |- Enabled => %s -> [GET] %s%s', name, cuksPkg.cfg.mount === '/' ? '' : cuksPkg.cfg.mount, routePath)
      return
    }
    let mws = []
    ...
    _.each(routes, r => {
      ...
      if (pkg.cfg.trimIndex && _.last(parts) === 'index') {
        ...
        realPath = '/' + parts.join('/') + pkg.cfg.ext
      }
      ...
      _.each(methods, m => {
        ...
        if (r.redirect) {
          ...
          helper('core:trace')('|  |  |- Enabled => %s -> [%s:Redirect] %s%s -> %s', name, m.toUpperCase(), cuksPkg.cfg.mount === '/' ? '' : cuksPkg.cfg.mount, realPath, dest)
        } else {
          ...
          helper('core:trace')('|  |  |- Enabled => %s -> [%s] %s%s', name, m.toUpperCase(), cuksPkg.cfg.mount === '/' ? '' : cuksPkg.cfg.mount, realPath)
        }
      })
    })
  }
